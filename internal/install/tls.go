...
func certificateFiles(now time.Time) (map[string][]byte, error) {
	...
	return map[string][]byte{
		...
		"ca-key.pem":     encode("RSA PRIVATE KEY", x509.MarshalPKCS1PrivateKey(caKey)),
...
func loadSecrets(p Paths) (credentials, error) {
	...
	for _, name := range []string{"ca.pem", "ca-key.pem", "server.pem", "server-key.pem", "credentials.json"} {
	...
	if len(creds.JWTSecret) != 64 || len(creds.DatabasePassphrase) != 64 {
