...
- **Ephemeral copy-on-write runners.** Each runner is a clone of a sealed, immutable, credential-free Ubuntu 24.04 template machine: per-clone CPU and memory limits (enforced via cgroups) and isolation are applied while the clone is still stopped, before first boot. Per-machine disk limits (`disk_bytes`) are enforced at the machine's btrfs subvolume (verified: incompressible data hits ENOSPC at the configured cap). Job workspaces and guest Docker state die with the machine.
