...
func TestSDKCreatePoolMismatchAndDuplicateExit(t *testing.T) {
	...
	manifest := templates.Manifest{SchemaVersion: 1, ImageID: "approved-image", MachineID: "template-id", OSVersion: "noble", Variant: "minimal", Arch: "arm64", RunnerFilename: "actions-runner-linux-arm64-2.333.0.tar.gz", RunnerSHA256: strings.Repeat("a", 64), RecipeSHA256: strings.Repeat("b", 64), OrbStackVersion: "2.2.3", Packages: map[string]string{"git": "1:2.43.0"}}
